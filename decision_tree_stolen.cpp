...
int main(int argc, const char *argv[]) {
	if (argc != 4) {
		cout << "Usage: dt.exe [train.csv] [test.csv] [result.csv]";
		return 0;
	}

	string trainFileName = argv[1];
	InputReader trainInputReader(trainFileName);
	Table trainTable = trainInputReader.getTable();

	// Here, you would initialize and train your DecisionTree class (not shown here for brevity)
	...
	outputPrinter.addLine(outputPrinter.joinByComma(test.attrName));
